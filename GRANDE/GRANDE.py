...
class GRANDE(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        node_result = tf.where(tf.math.is_nan(node_result), tf.zeros_like(node_result), node_result)
        ...
        estimator_weights_leaf = tf.where(tf.math.is_nan(estimator_weights_leaf), tf.zeros_like(estimator_weights_leaf), estimator_weights_leaf)
    ...
    def perform_preprocessing(self, 
        ...
        all_nan_columns = X_train.isna().all(axis=0)       
        X_train.loc[:, all_nan_columns] = X_train.loc[:, all_nan_columns].fillna(0)
